...
function tree_node_ward() {
    return {
        kind : "ward",
        key : "ward",
        icon : "bed",
        // ПАКЕТ №345: палата даёт одну койку.
        effect : "палата на одну койку"
    };
}
...
/// @description Ветки панели РАЗВИТИЕ.
///
/// ПАКЕТ №325: ветки собираются от данных ТЕКУЩЕЙ клиники, а не общим
/// списком. У клиники с двумя кабинетами нет третьего кабинета в
/// панели, у клиники без стационара нет ни палаты, ни коек, у клиники
/// без операционной — операционной. Потолки берутся из
/// clinic_exam_room_limit / clinic_bed_limit / clinic_has_operating_room.
function tree_sections() {

    var _sections = [];

    // ── Общая ветка: есть у всех клиник ──
    ...
    // ── ПРИЁМ: кабинеты от второго до потолка клиники ──
    ...
    for (var _slot = 2; _slot <= _exam_limit; _slot++) {
        array_push(
            _exam_nodes,
            tree_node_room(_slot, "door", "стол приёма " + string(_slot))
        );
    }

    if (array_length(_exam_nodes) > 0) {
        array_push(_sections, {
            title : "ПРИЁМ",
            nodes : _exam_nodes
        });
    ...
    // ── СТАЦИОНАР: палата плюс койки сверх одной (№345) ──
    var _bed_limit = script_exists(asset_get_index("clinic_bed_limit"))
        ? clinic_bed_limit()
        : 0;

    if (_bed_limit > 0) {
        var _bed_nodes = [tree_node_ward()];

        for (var _bed = 102; _bed < 101 + _bed_limit; _bed++) {
            array_push(_bed_nodes, tree_node_bed(_bed));
        }
    ...
    // ── ОПЕРАЦИОННАЯ: только там, где она предусмотрена ──
    if (
...
function tree_node_state(_node) {
    switch (_node.kind) {
        ...
        case "room": {
            ...
            if (round(_node.key) >= 3 && !clinic_exam_room_is_open(round(_node.key) - 1)) {
                return "lock";
            }
            return "open";
        }

        case "ward": return clinic_ward_is_open() ? "done" : "open";

        case "bed": {
            if (clinic_bed_is_open(_node.key)) return "done";
            if (!clinic_ward_is_open()) return "lock";
            if (round(_node.key) >= 103 && !clinic_bed_is_open(round(_node.key) - 1)) {
                return "lock";
            }
            return "open";
        }
...
function tree_node_title(_node) {
    switch (_node.kind) {
        ...
        case "ward": return clinic_room_name(_node.key);
