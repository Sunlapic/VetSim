...
registration_duration = game_get_speed(gamespeed_fps) * 2;
...
player_begin_service = function(_owner, _mode) {
    ...
    if (!instance_exists(_free_table)) {
        if (instance_exists(obj_UI_HUD)) {
            with (obj_UI_HUD) {
                show_notice("НЕТ СВОБОДНОГО СТОЛА", "Все смотровые столы заняты.", game_get_speed(gamespeed_fps) * 3);
            }
        }
        return false;
    }
...
if (_case_confirmed) {
        ...
        if (_case_confirmed && array_length(_next_assignments) > 0) {
            ...
            if (_wait_index != -1) {
               ...
               with (_owner_done) {
                ...
                if (instance_exists(obj_UI_HUD)) {
                    with (obj_UI_HUD) {
                        show_notice("НАЗНАЧЕНИЯ ГОТОВЫ", "Пациент переведён в очередь на процедуры.", game_get_speed(gamespeed_fps) * 3);
                    }
                }
    ...
    if (instance_exists(obj_UI_HUD)) {
        with (obj_UI_HUD) {
            show_notice("НА ОПЛАТУ", "Клиент направлен к стойке для расчёта.", game_get_speed(gamespeed_fps) * 3);
        }
    }
    ...
    if (instance_exists(obj_UI_HUD)) {
        with (obj_UI_HUD) {
            if (_next_proc_id != "") {
                ...
                show_notice("НА ОПЛАТУ", "Процедуры выполнены. Клиент идёт к стойке.", game_get_speed(gamespeed_fps) * 3);
            } else if (_next_doctor_id != "") {
                show_notice("НА ОПЛАТУ", "Процедуры выполнены. Клиент идёт к стойке, затем будет контроль у врача.", game_get_speed(gamespeed_fps) * 3);
            } else {
                show_notice("НА ОПЛАТУ", "Процедуры выполнены. Клиент идёт к стойке для расчёта.", game_get_speed(gamespeed_fps) * 3);
            }
        }
    }
