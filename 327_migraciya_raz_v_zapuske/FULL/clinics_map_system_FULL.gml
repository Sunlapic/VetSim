...
function clinics_init() {
    ...
    if (
        ...
        !variable_global_exists("clinics_spec_migrated")
        || !global.clinics_spec_migrated
    ) {
        // ПАКЕТ №327: миграция выполняется ОДИН раз за запуск игры.
        //
        // clinics_init вызывают в том числе из obj_UI_HUD -> Step, то
        // есть каждый кадр. Без этого флага clinics_migrate_to_spec
        // пересобирала global.clinics и писала в консоль на каждом
        // кадре — сотни одинаковых строк за минуту.
        clinics_migrate_to_spec();

        global.clinics_spec_migrated = true;
    }
