...
function save_slot_new_game() {
    ...
    // ═══════════════════════════════════════════════════════════
    // ПАКЕТ №329: ЯВНЫЙ СБРОС И ПРОВЕРКА, ЧТО ФАЙЛ УДАЛЁН
    //
    // До этого «новая игра» только удаляла файл и надеялась на
    // game_restart. Теперь показатели возвращаются к стартовым
    // явно, а удаление файла проверяется: если он всё ещё на диске,
    // после перезапуска загрузилась бы старая партия, и в консоли
    // была бы строка с причиной.
    //
    // new_game_reset печатает стартовые значения — деньги, баллы,
    // репутацию, день. game_restart ниже всё равно обнулит global
    // целиком (несколько сотен переменных за день игры иначе не
    // вычистить), но сброс здесь делает старт видимым и не даёт
    // забытой переменной пережить перезапуск.
    // ═══════════════════════════════════════════════════════════
    if (script_exists(asset_get_index("new_game_reset"))) {
        new_game_reset();
    }
    else {
        show_debug_message(
            "[SLOT] ВНИМАНИЕ: скрипт vet_new_game не создан в проекте — "
            + "сброс выполнен только перезапуском."
        );
    }

    if (file_exists(SAVE_FILE_NAME)) {
        show_debug_message(
            "[SLOT] ВНИМАНИЕ: файл сохранения " + SAVE_FILE_NAME
            + " не удалился — после перезапуска загрузится старая игра."
        );
    }
    else {
        show_debug_message("[SLOT] Файл сохранения удалён — старт с нуля.");
    }
