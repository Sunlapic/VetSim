...
function actor_draw_action_progress() {
    ...
    //
    // ПАКЕТ №330: рисуется той же функцией, что и шкала ожидания над
    // головой клиента, — тот же вариант оформления (WAIT_BAR_STYLE),
    // та же тонкая чёрная обводка и та же пустая дорожка, по которой
    // видно, сколько уже прошло.
    if (_has_bar) {
        ...
        if (script_exists(asset_get_index("draw_wait_bar"))) {
            draw_wait_bar(
                _bar_x1, _bar_y1, _bar_x2, _bar_y2,
                _ratio, _bar_color,
                -1
            );
        }
        else {
            // Запасной путь, если скрипт wait_bar_system не создан.
