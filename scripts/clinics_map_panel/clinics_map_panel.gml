...
// clinics_map_panel
//
// Экран карты: сама карта с шестью зданиями и карточка клиники,
...
function clinics_map_building_rect(_hud, _index) {
    ...
    if (is_struct(_clinic) && variable_struct_exists(_clinic, "exam_rooms")) {
        _size_step = clamp(_clinic.exam_rooms - 1, 0, 4);
    }
...
function hud_draw_clinic_card(_hud) {
    ...
    var _rows = [
        ["Смотровые кабинеты", string(_clinic.exam_rooms)],
        ["Койки стационара", (_clinic.beds > 0) ? string(_clinic.beds) : "нет"],
        ["Операционная", _clinic.has_operating ? "есть" : "нет"],
        ["Доход в день", "$" + string(_clinic.income_per_day)]
    ];

    if (_clinic.owned) {
        array_push(_rows, ["Репутация", string(_clinic.reputation)]);
    }
    else {
