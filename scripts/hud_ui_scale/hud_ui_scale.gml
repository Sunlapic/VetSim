...
// ═══════════════════════════════════════════════════════════════
// ОБРЕЗКА ПО ОБЛАСТИ (пакет №209)
//
// До этого во всех списках действовало правило «рисуем только то, что
// влезло целиком»: при прокрутке следующая строка появлялась рывком, и
// было не видно, что ниже что-то есть.
//
// Теперь список обрезается по краю окна: наполовину выехавшая строка так
// и рисуется — половинкой. Сразу понятно, что список можно крутить дальше.
//
// Координаты GUI переводятся в пиксели окна, поэтому обрезка не съезжает
// при масштабировании интерфейса на телефоне.
// ═══════════════════════════════════════════════════════════════

function ui_clip_begin(_x1, _y1, _x2, _y2) {
    var _gw = max(1, display_get_gui_width());
    var _gh = max(1, display_get_gui_height());
    var _sx = window_get_width() / _gw;
    var _sy = window_get_height() / _gh;

    if (_sx <= 0 || _sy <= 0) return;

    var _left = floor(min(_x1, _x2) * _sx);
    var _top = floor(min(_y1, _y2) * _sy);
    var _w = ceil(abs(_x2 - _x1) * _sx);
    var _h = ceil(abs(_y2 - _y1) * _sy);

    gpu_set_scissor(max(0, _left), max(0, _top), max(0, _w), max(0, _h));
}

function ui_clip_end() {
