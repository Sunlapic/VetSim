...
function animal_perform_diagnostic(_animal_id, _diagnostic_id) {
    ...
    // ПАКЕТ №318: приём засчитывается в итоги дня.
    //
    // Через эту функцию игрок обследует пациента. Раньше строка
    // «Принято пациентов» показывала число ОПЛАТ на стойке, поэтому
    // осмотренные игроком пациенты в неё не попадали, и цифра не
    // сходилась с количеством диагнозов.
    //
    // Функция сама защищена от повторов: несколько обследований
    // одного пациента дают один приём.
