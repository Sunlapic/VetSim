...
// ═══════════════════════════════════════════════════════════════
// Пакет №268: ФИЛЬТР БОЛЕЗНЕЙ ПО ОТКРЫТЫМ ПОМЕЩЕНИЯМ
//
// Болезни, требующие операционной или стационара, не приходят в клинику,
// пока соответствующее помещение не открыто (см. скрипт clinic_case_gate).
//
// Здесь два независимых переключателя, чтобы удобно проверять механику:
//
//   CLINIC_CASE_GATE_ENABLED
//     false — фильтр полностью выключен, болезни идут как раньше.
//     Пригодится, если понадобится быстро сравнить поведение.
//
//   CLINIC_GATE_IGNORE_TESTING_UNLOCK
//     Выше стоит CLINIC_OPERATING_FREE_WHILE_TESTING = true — операционная
//     считается открытой бесплатно, пока игра в разработке. Значит и
//     хирургические случаи будут приходить, то есть проверить фильтр
//     операционной не выйдет, не отключив тестовый режим.
//
//     Поставьте здесь true — и ФИЛЬТР начнёт смотреть на реальную покупку
//     операционной, при этом сама операционная останется доступной для
//     тестов. Это позволяет проверить «не приходят хирургические случаи»
//     не трогая CLINIC_OPERATING_FREE_WHILE_TESTING.
// ═══════════════════════════════════════════════════════════════
...
/// Нужна фильтру болезней (пакет №268).
