...
function reception_finish_owner_payment(_owner_inst) {
    ...
    // ═══════════════════════════════════════════════════════════
    // ЗЕЛЁНАЯ НАДПИСЬ О ДОХОДЕ: «+25$»
    //
    // Показываем ровно в момент, когда деньги легли в кассу, и
    // ровно на том месте, где это произошло.
    //
    // Надпись привязана к СТОЙКЕ, а не к клиенту: сразу после
    // оплаты клиент разворачивается и уходит к выходу, и надпись
    // уехала бы вместе с ним через полкомнаты. Если стойку почему-то
    // не нашли, показываем над самим клиентом — это лучше, чем
    // ничего.
    // ═══════════════════════════════════════════════════════════
    ...
    if (_pay_amount > 0) {
        if (instance_exists(_desk)) {
            ...
            float_text_money(_owner_inst, _pay_amount);
        }
    }
