...
if (!variable_instance_exists(id, "STUCK_THRESHOLD")) STUCK_THRESHOLD = game_get_speed(gamespeed_fps) * 2;

if (!variable_instance_exists(id, "wander_walking")) wander_walking = false;
if (!variable_instance_exists(id, "wander_idle_timer")) wander_idle_timer = game_get_speed(gamespeed_fps) * 2;
...
if (
    ...
    registration_duration = round(
        game_get_speed(gamespeed_fps) * lerp(10, 1, (_player_admin_level - 1) / 9)
    );
}
...
if (_actually_walking && object_index != obj_staff_candidate) {
    ...
    if (walk_skill_timer >= game_get_speed(gamespeed_fps) * 30) {
        walk_skill_timer -= game_get_speed(gamespeed_fps) * 30;
...
if (_run_stuck_protection) {
    ...
    if (_is_doctor_like && instance_exists(assigned_owner)) {
        if (
            ...
            _stuck_threshold = game_get_speed(gamespeed_fps) * 0.30;
        }
    }

    if (string_copy(_current_state, 1, 7) == "restock") {
        _stuck_threshold = max(_stuck_threshold, game_get_speed(gamespeed_fps));
    }
...
if (object_index == obj_staff_candidate) {
    ...
    if (
        ...
        _speech_cooldown = game_get_speed(gamespeed_fps) * 45;
    }
    else if (
        is_tired
        && !_speech_last_tired
