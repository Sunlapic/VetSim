...
if (_is_idle && _is_assistant) {
    ...
    // ═══════════════════════════════════════════════════════════════
    // ПАКЕТ 277 (задача 1): СКАН СКЛАДА НЕ КАЖДЫЙ КАДР
    //
    // Здесь restock_scan_needs() вызывался КАЖДЫЙ кадр у каждого
    // свободного ассистента. Одно сканирование — это все шкафы,
    // умноженные на 44 препарата, и для каждой пары ещё проход по
    // очереди заданий и по всем ассистентам. Шестьдесят раз в
    // секунду. Отсюда просадка кадров в спокойное время, когда
    // ассистенты как раз простаивают.
    //
    // В obj_Render точно такой же вызов стоит раз в 15 кадров —
    // там защиту поставили, а здесь забыли. Ставлю тот же счётчик.
    //
    // Стартовое значение случайное: если ассистентов несколько, их
    // сканирования расходятся по разным кадрам и не собираются в
    // один тяжёлый кадр.
    // ═══════════════════════════════════════════════════════════════

    if (!variable_instance_exists(id, "restock_scan_tick")) {
        restock_scan_tick = irandom(14);
    }

    restock_scan_tick += 1;

    if (restock_scan_tick >= 15) {
        restock_scan_tick = 0;
        restock_scan_needs();
    }
...
if (_is_idle) {
    ...
    if (!wander_walking && wander_idle_timer <= 0) {
        ...
        wander_idle_timer = game_get_speed(gamespeed_fps) * (1 + irandom(2));
    }
    if (wander_walking && !is_walking) {
        wander_walking    = false;
        wander_idle_timer = game_get_speed(gamespeed_fps) * (2 + irandom(4));
    }
} else {
    if (wander_walking) {
    ...
    wander_walking    = false;
    wander_idle_timer = game_get_speed(gamespeed_fps) * (2 + irandom(3));
}
