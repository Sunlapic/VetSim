...
function clinic_room_audit() {
    ...
    // ── Зоны уборки ──
    // Первая зона обязательна. Вторая — только если в комнате
    // поставили хоть один из пары start_2/end_2 (клиника №3
    // спокойно живёт с одной зоной).
    _problems = clinic_audit_object("obj_cleanliness_area_start", 1, _problems);
    _problems = clinic_audit_object("obj_cleanliness_area_end", 1, _problems);

    var _area2_start = asset_get_index("obj_cleanliness_area_start_2");
    var _area2_end = asset_get_index("obj_cleanliness_area_end_2");
    var _area2_used = false;

    if (_area2_start != -1 && object_exists(_area2_start) && instance_number(_area2_start) > 0) {
        _area2_used = true;
    }

    if (_area2_end != -1 && object_exists(_area2_end) && instance_number(_area2_end) > 0) {
        _area2_used = true;
    }

    if (_area2_used) {
        _problems = clinic_audit_object("obj_cleanliness_area_start_2", 1, _problems);
        _problems = clinic_audit_object("obj_cleanliness_area_end_2", 1, _problems);
    }

    // ── Столы: два стола на один слот ──
    //
    // Раньше любой стол со слотом 1 считался «без Creation Code».
    // У кабинета 1 слот как раз 1 — ложные срабатывания.
    // Теперь ругаемся, только если на один слот повешено больше
    // одного стола (забыли прописать exam_slot_id у второго).
    var _table_assets = ["obj_table", "obj_table_1"];
    var _slot_counts = {};

    for (var _index = 0; _index < array_length(_table_assets); _index++) {
        var _asset = asset_get_index(_table_assets[_index]);
        ...
        for (var _k = 0; _k < _total; _k++) {
            ...
            var _slot_key = string(round(_inst.exam_slot_id));
            var _was = 0;

            if (variable_struct_exists(_slot_counts, _slot_key)) {
                _was = variable_struct_get(_slot_counts, _slot_key);
            }

            variable_struct_set(_slot_counts, _slot_key, _was + 1);

            if (_was + 1 == 2) {
                show_debug_message(clinic_audit_msg(
                    "два стола со слотом " + _slot_key
                    + " — у одного из них в Creation Code нужен свой exam_slot_id."
                ));
    ...
    // ── Регистратура ──
    // Обязательна одна полная стойка (слот 1 или любой занятый).
    // Вторая стойка не обязательна: клиника №3 живёт с одной.
    // Если слот 2 частично расставлен — проверяем его целиком.
    ...
    var _reception_slots_found = 0;

    for (var _slot = 1; _slot <= 2; _slot++) {
        var _any = 0;

        for (var _index = 0; _index < array_length(_reception_objects); _index++) {
            _any += clinic_count_by_slot(_reception_objects[_index], _slot);
        }

        if (_any <= 0) continue;

        _reception_slots_found += 1;
        _problems = clinic_audit_group(
            _slot,
            _reception_objects,
    ...
    if (_reception_slots_found <= 0) {
        var _desk_asset = asset_get_index("obj_reception_desk");
        var _desk_n = (
            _desk_asset != -1 && object_exists(_desk_asset)
        ) ? instance_number(_desk_asset) : 0;

        if (_desk_n > 0) {
            show_debug_message(clinic_audit_msg(
                "стойка регистратуры стоит, но без reception_slot_id. "
                + "В Creation Code инстанса стойки и обеих точек "
                + "(obj_reception_point_staff и obj_reception_point_owner) "
                + "пропиши reception_slot_id = 1;"
            ));
        }
        else {
            show_debug_message(clinic_audit_msg(
                "нет регистратуры. Нужна одна стойка и две точки "
                + "(персонал и клиент), у всех reception_slot_id = 1;"
            ));
        }
