...
/// @function clinic_development_bonus(_clinic)
/// @description ПАКЕТ №346: чем сильнее развита клиника, тем выше
///              её пассивный доход. Каждая купленная комната даёт
///              прибавку к базовому income_per_day:
///                кабинет приёма (сверх первого)  +200/день;
///                палата стационара                +300/день;
///                каждая койка                     +150/день;
///                операционная                     +600/день.
function clinic_development_bonus(_clinic) {
    if (!is_struct(_clinic)) return 0;
    if (!variable_struct_exists(_clinic, "id")) return 0;

    var _rooms = undefined;

    if (
        _clinic.id == clinic_current_id()
        && variable_global_exists("clinic_rooms_open")
        && is_struct(global.clinic_rooms_open)
    ) {
        _rooms = global.clinic_rooms_open;
    }
    else {
        var _state = clinic_state_get(_clinic.id);

        if (is_struct(_state.rooms)) _rooms = _state.rooms;
    }

    if (!is_struct(_rooms)) return 0;

    var _bonus = 0;
    var _names = variable_struct_get_names(_rooms);

    for (var _i = 0; _i < array_length(_names); _i++) {
        var _key = _names[_i];

        if (variable_struct_get(_rooms, _key) != true) continue;
        ...
        else if (string_pos("bed_", _key) == 1) {
            _bonus += 150;
        }
        else if (string_digits(_key) == _key && _key != "") {
            // Кабинеты приёма: первый базовый, каждый следующий +200.
            _bonus += 200;
        }
    }
...
/// @function clinic_passive_income(_clinic)
/// @description Пассивный доход клиники в день: база из данных
///              клиники плюс бонус развития (пакет №346).
function clinic_passive_income(_clinic) {
    var _base = variable_struct_exists(_clinic, "income_per_day")
        ? _clinic.income_per_day
        : 0;

    return _base + clinic_development_bonus(_clinic);
}


/// @function clinic_staff_quality(_state)
/// @description ПАКЕТ №347: качество персонала клиники.
///
/// Доход зависит не только от того, СКОЛЬКО людей в клинике, но и
/// КАКИЕ они: средний профильный уровень штата превращается
/// в множитель дохода.
///
///   уровень 1  → 0.76 (новички еле тянут)
///   уровень 5  → 1.00 (норма)
///   уровень 10 → 1.30 (мастера приносят на 30% больше)
///
/// Пустой штат → 0.7, но доход всё равно обнуляется раньше —
/// без людей клиника не работает вовсе.
function clinic_staff_quality(_state) {
    if (!variable_struct_exists(_state, "staff")) return 0.7;

    var _sum = 0;
    var _count = 0;

    for (var _i = 0; _i < array_length(_state.staff); _i++) {
        var _snap = _state.staff[_i];
        if (!is_struct(_snap)) continue;

        _sum += staff_main_skill_average(_snap);
        _count += 1;
    }

    if (_count <= 0) return 0.7;

    return 0.7 + 0.06 * (_sum / _count);
}
...
function clinic_network_daily_income() {
    ...
    for (var _index = 0; _index < array_length(global.clinics); _index++) {
        ...
        // ПАКЕТ №346: база + бонус за развитие клиники.
        var _income = clinic_passive_income(_clinic);
        ...
        // ПАКЕТ №347: качество персонала. Слабый штат приносит
        // меньше, прокачанный — больше. Количество людей задаёт
        // _ratio, их уровень — _quality.
        var _quality = clinic_staff_quality(_state);

        _total += floor(_income * _ratio * _quality);
    }
