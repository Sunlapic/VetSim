...
function save_staff_snapshot(_staff) {
    ...
    return {
        ...
        // ПАКЕТ 347: уровни навыков на момент найма — по ним
        // просьба о прибавке называет конкретный выросший навык.
        hire_skills : save_copy_array_asis(_staff, "hire_skills"),
...
function save_staff_restore(_data) {
    ...
    with (_staff) {
        ...
        // ПАКЕТ 347: снимок навыков на момент найма. В старых
        // сохранениях его нет — тогда считаем, что сотрудник не
        // вырос ни в одном конкретном навыке.
        if (variable_struct_exists(_data, "hire_skills")
            && is_array(_data.hire_skills)
        ) {
            hire_skills = [];
            for (var _hi = 0; _hi < array_length(_data.hire_skills);
                _hi++
            ) {
                hire_skills[_hi] = _data.hire_skills[_hi];
            }
        }
...
function save_build_data() {
    var _data = {
        ...
        clinic_points : save_global("clinic_points", 500),
        // ПАКЕТ №325: запасное значение 0 — репутация любой клиники
        // стартует с нуля, правда всё равно лежит в кармане клиники.
        clinic_reputation : save_global("clinic_reputation", 0),
...
function vetsim_load() {
    ...
    global.clinic_points = save_field(_data, "clinic_points", save_global("clinic_points", 500));
    global.clinic_reputation = save_field(
        _data, "clinic_reputation", save_global("clinic_reputation", 0)
    );
