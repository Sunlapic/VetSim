...
function tablet_add_fly_effect(_start_x, _start_y, _end_x, _end_y, _symbol, _color) {
    array_push(fly_effects, {
        ...
        timer : 0,
        timer_max : max(1, round(game_get_speed(gamespeed_fps) * 0.65)),
    ...
    for (var d = 0; d < array_length(_diag_list); d++) {
        ...
        if (_doctor_assign_mode && tablet_click_lock <= 0 && _hover_diag && mouse_check_button_pressed(mb_left)) {
            ...
            if (instance_exists(_hud_inst)) {
                _hud_inst.show_notice("ДИАГНОСТИКА", db_get_diagnostic_name(_diag_id), game_get_speed(gamespeed_fps) * 2);
            }
...
if (_doctor_assign_mode) {
        ...
        if (_can_press && tablet_click_lock <= 0 && _hover_treat && mouse_check_button_pressed(mb_left)) {
            ...
            if (_is_correct) {
                ...
                if (instance_exists(_hud_ok)) {
                    _hud_ok.show_notice("НАЗНАЧЕНО", db_get_treatment_action_name(_action_id), game_get_speed(gamespeed_fps) * 2);
                }
            } else {
                ...
                if (instance_exists(_hud_bad)) {
                    _hud_bad.show_notice("ОШИБКА НАЗНАЧЕНИЯ", db_get_treatment_action_name(_action_id), game_get_speed(gamespeed_fps) * 2);
                }
            }
    ...
    if (instance_exists(_hud_proc)) {
        _hud_proc.show_notice("ПРОЦЕДУРА", db_get_treatment_action_name(_proc_action_id), game_get_speed(gamespeed_fps) * 2);
    }
