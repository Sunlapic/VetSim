...
if (_now_minute == 0 && render_last_close_day != global.game_day) {
    ...
    if (instance_exists(obj_owner)) {
        ...
        global.day_summary_wait_frames = game_get_speed(gamespeed_fps) * 2;
...
// ═══════════════════════════════════════════════════════════════
// 3.2 ПАКЕТ №329: ТЕСТОВАЯ КОМНАТА ИЗ ИГРЫ УБРАНА
//
// Здесь была клавиша R — переход в Room1 «посмотреть все системы
// разом». Room1 теперь только база объектов: из неё расставляются
// четыре клиники, и в игре она участвовать не должна.
//
// Убрано:
//   - переход в Room1 по клавише R;
//   - сворачивание клиники в карман перед таким переходом.
//
// Если в техническую комнату всё же попасть (например, сделать её
// стартовой и убрать obj_Render), obj_Render → Create уводит игру
// обратно в клинику №1, а диагностика в ней молчит.
//
// Клавиша R освобождена.
// ═══════════════════════════════════════════════════════════════
...
if (render_last_day != global.game_day) {
    ...
    // ── ПАКЕТ №347: НЕАКТИВНЫЕ КЛИНИКИ ЖИВУТ.
    //
    // Сотрудники неактивных клиник понемногу прокачиваются
    // (пункты опыта, не уровни) и могут попросить прибавку —
    // просьба уходит в общий колокольчик. Просроченные просьбы
    // (3 дня без ответа) отклоняются сами.
    //
    // Идёт ДО начисления дохода, чтобы множитель качества
    // персонала считался по свежим навыкам.
    if (script_exists(asset_get_index("staff_idle_daily_tick"))) {
        staff_idle_daily_tick();
    }
    if (script_exists(asset_get_index("staff_requests_expire"))) {
        staff_requests_expire();
    }
        ...
        if (_raise_asked > 0 && instance_exists(obj_UI_HUD)) {
            with (instance_find(obj_UI_HUD, 0)) {
                show_notice(
                    "РАЗГОВОР О ЗАРПЛАТЕ",
                    string(_raise_asked)
                    + " сотр. просит прибавку — откройте колокольчик",
