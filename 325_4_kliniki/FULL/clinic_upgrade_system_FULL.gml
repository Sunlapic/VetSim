...
/// @description Баллы клиники и уровни улучшений: слоты найма,
///              библиотека, спортзал, аптека.
/// Пакет №71, №121, №122, №173.
///
/// ПАКЕТ №325: кошелёк баллов (global.clinic_points) остаётся ОБЩИМ
/// на всю сеть — так решили в дизайне. А вот уровни улучшений
/// (global.clinic_upgrades) у каждой клиники свои: их подменяет
/// clinic_apply_current из кармана клиники, поэтому прокачка второй
/// клиники не прокачивает первую.
///
/// Потолок ветки «Слот найма» теперь не общий макрос, а данные
/// клиники: hire_max 4, 7, 10 и 17 у клиник 1–4 соответственно.
...
/// Общий потолок для улучшений без собственного лимита.
function clinic_upgrade_max_level() {
    return 5;
}


/// Потолок конкретной ветки в конкретной клинике.
///
/// ПАКЕТ №325: «Слот найма» растёт ровно до штата клиники: hire_max
/// минус один стартовый сотрудник. В клинике на 4 человека ветка
/// заканчивается на третьем уровне, в клинике на 17 — на шестнадцатом.
function clinic_upgrade_max_level_for(_upgrade_id) {
    if (string(_upgrade_id) == "hire_slot") {
        if (script_exists(asset_get_index("clinic_hire_max_level"))) {
            return clinic_hire_max_level();
        }

        return TREE_HIRE_MAX_LEVEL;
    }
...
///
/// Цены «Слота найма» заданы до 16 уровня: столько нужно самой
/// крупной клинике сети (17 сотрудников). Ветке маленькой клиники
/// дальше третьего уровня просто не дойти — её потолок ниже.
...
/// Сколько сотрудников можно нанять в этой клинике.
///
/// Стартовый лимит — 1 сотрудник (не считая главного игрока). Дальше
/// по уровню ветки, но никогда не выше hire_max клиники.
function clinic_get_hire_slots() {
    var _slots = 1 + clinic_upgrade_level("hire_slot");

    if (script_exists(asset_get_index("clinic_hire_max"))) {
        _slots = min(_slots, clinic_hire_max());
    }
...
function hud_draw_clinic_upgrades(_hud) {
    // Панель РАЗВИТИЕ — дерево веток, вся отрисовка и клики в
    // Script Asset clinic_tree_panel. Ветки собираются от данных
    // текущей клиники, поэтому пунктов сверх её потолков там нет.
