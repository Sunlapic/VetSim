...
// Койки стационара: 101 и 102 идут в комплекте с палатой, остальные
// докупаются по одной.
...
/// стационар (bed_max > 0) и номер койки не выше её потолка. Первые
/// две койки открываются вместе с палатой, остальные докупаются.
function clinic_bed_is_open(_slot_id) {
    ...
    if (_slot == 101 || _slot == 102) return clinic_ward_is_open();
...
function clinic_room_purchase(_slot_id) {
    ...
    if (_is_operating) {
        ...
        // Палата открывает первые две койки сразу.
        variable_struct_set(global.clinic_rooms_open, "bed_101", true);
        variable_struct_set(global.clinic_rooms_open, "bed_102", true);

        clinic_room_free_beds([101, 102]);
    }
