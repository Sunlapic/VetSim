...
/// @description Чёрный контур капсулы: два круга-обводки на торцах
///              и две горизонтальные линии сверху и снизу. Не
///              зависит от длины заливки — рисуется по ВСЕЙ капсуле.
function wait_bar_capsule_outline(_x1, _y1, _x2, _y2, _t) {
    ...
    var _lx = _x1 + (_y2 - _y1) * 0.5;
    var _rx = _x2 - (_y2 - _y1) * 0.5;
    ...
    // ПАКЕТ №334 (fix): только ВНЕШНИЕ полуокружности. Полный
    // draw_circle оставлял дугу внутри капсулы — лишнее кольцо.
    // draw_arc: 90..270 — левая половина, 270..450 — правая.
    draw_arc(_lx, _cy, _r, 90, 270, true);
    draw_arc(_rx, _cy, _r, 270, 450, true);

    draw_set_alpha(1);
    draw_line(_lx, _y1 - _t, _rx, _y1 - _t);
    draw_line(_lx, _y2 + _t, _rx, _y2 + _t);
...
// ═══════════════════════════════════════════════════════════════
// 4. ВАРИАНТЫ ОФОРМЛЕНИЯ (№334: капсула из кругов, обводка постоянная)
// ═══════════════════════════════════════════════════════════════
...
function wait_bar_style_capsule(_pos, _ratio, _color) {
    ...
    wait_bar_capsule_outline(_pos.x1, _pos.y1, _pos.x2, _pos.y2, 1);
...
function wait_bar_style_line(_pos, _ratio, _color) {
    ...
    wait_bar_capsule_outline(_thin.x1, _thin.y1, _thin.x2, _thin.y2, 1);
