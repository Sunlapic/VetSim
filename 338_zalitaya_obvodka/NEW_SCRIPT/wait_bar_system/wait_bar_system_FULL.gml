...
///              ПАКЕТ №337: БЕЗ отступа — при полной полоске цвет
///              касается контура, пустоты нет; длина по остатку
///              терпения, пустеет слева направо внутри постоянного
///              контура.
...
/// @function wait_bar_ring(_cx, _cy, _r1, _r2, _a1, _a2)
/// @description ЗАЛИТОЕ полукольцо между радиусами _r1 и _r2,
///              собранное из треугольников. ПАКЕТ №338: на маштабе
///              пользователя draw_line рисует 1 ЭКРАННЫЙ пиксель,
///              поэтому «лента из штрихов» распадалась на две
///              тонкие линии с щелью. Залитые примитивы
///              (прямоугольники и треугольники) масштабируются
///              вместе с координатами — толщина честная.
function wait_bar_ring(_cx, _cy, _r1, _r2, _a1, _a2) {
    ...
    for (var _i = 0; _i < _steps; _i++) {
        ...
        var _b = _a1 + (_a2 - _a1) * (_i + 1) / _steps;

        draw_vertex(_cx + lengthdir_x(_r1, _a), _cy + lengthdir_y(_r1, _a));
        draw_vertex(_cx + lengthdir_x(_r2, _a), _cy + lengthdir_y(_r2, _a));
        draw_vertex(_cx + lengthdir_x(_r1, _b), _cy + lengthdir_y(_r1, _b));

        draw_vertex(_cx + lengthdir_x(_r2, _a), _cy + lengthdir_y(_r2, _a));
        draw_vertex(_cx + lengthdir_x(_r2, _b), _cy + lengthdir_y(_r2, _b));
        draw_vertex(_cx + lengthdir_x(_r1, _b), _cy + lengthdir_y(_r1, _b));
    }
...
/// @function wait_bar_capsule_band(_x1, _y1, _x2, _y2, _o1, _o2)
/// @description ЗАЛИТАЯ лента вокруг капсулы между отступами
///              _o1.._o2: прямоугольники сверху/снизу + полукольца
///              на торцах. Рисует текущим цветом/альфой.
function wait_bar_capsule_band(_x1, _y1, _x2, _y2, _o1, _o2) {
    ...
    if (_rx <= _lx) {
        // Капсула короче диаметра: полное кольцо из двух половин.
        wait_bar_ring(_lx, _cy, _r + _o1 - 1, _r + _o2, 90, 270);
        wait_bar_ring(_rx, _cy, _r + _o1 - 1, _r + _o2, 270, 450);
        return true;
    }

    draw_rectangle(_lx, _y1 - _o2, _rx, _y1 - _o1, false);
    draw_rectangle(_lx, _y2 + _o1, _rx, _y2 + _o2, false);
...
///              линии сверху/снизу. ПАКЕТ №337: стили передают бокс
///              ВСЕЙ капсулы — контур постоянный, не укорачивается;
///              цвет внутри при полной полоске касается его без
///              пустоты. Толщина _t: штрихи 1.._t сливаются в ленту.
///              Дуги — ломаные (№335).
function wait_bar_capsule_outline(_x1, _y1, _x2, _y2, _t) {
    ...
    // №338: залитая лента толщиной _t (смещения 1.._t).
    wait_bar_capsule_band(_x1, _y1, _x2, _y2, 1, _t);
...
// ═══════════════════════════════════════════════════════════════
// 4. ВАРИАНТЫ ОФОРМЛЕНИЯ (№337: контур постоянный толщиной 2, цвет впритык)
// ═══════════════════════════════════════════════════════════════
...
function wait_bar_style_capsule(_pos, _ratio, _color) {
    ...
    // №337: контур ПОСТОЯННЫЙ по всей капсуле (не меняется при
    // уменьшении цвета), толщина 2; цвет прижат к нему впритык.
    wait_bar_capsule_outline(_pos.x1, _pos.y1, _pos.x2, _pos.y2, 2);
...
function wait_bar_capsule_outline_glow(_fill, _grow) {
    ...
    // №338: свечение — залитая лента 1.._grow вокруг заливки.
    wait_bar_capsule_band(_fill.x1, _fill.y1, _fill.x2, _fill.y2, 1, _grow);
...
function wait_bar_style_line(_pos, _ratio, _color) {
    ...
    // №337: контур постоянный, толщина 2.
    wait_bar_capsule_outline(_thin.x1, _thin.y1, _thin.x2, _thin.y2, 2);
