...
/// Пакет №185: подпись «Место работы:» переехала внутрь кнопки («МЕСТО: СТАЦИОНАР»).
/// Пакет №186: под именем пишется профессия («АНЕСТЕЗИОЛОГ»), из места работы
/// убрано лишнее слово «ОПЕРАЦИОННАЯ».
/// Пакет №217: подсказка внизу карточки показывает точные цифры выбранного
/// навыка на текущем уровне: уровень, опыт до следующего уровня, качество
/// и скорость выполнения (столбцы СКОРОСТЬ и КАЧЕСТВО из таблицы убраны).
...
function tablet_draw_staff_card(
    ...
    //
    // Пакет №217: подсказка показывает точные цифры выбранного навыка
    // на текущем уровне — уровень, опыт до следующего уровня, качество
    // и скорость выполнения. Раньше эти цифры стояли столбцами
    // в таблице навыков и делали строки мелкими и разными по шрифту.
    // ═══════════════════════════════════════════════════════════
    ...
    if (_help_id == "") {
        ...
        draw_text_ext_transformed(
            ...
            "Нажмите на навык: описание, качество и скорость на текущем уровне.",
            7 * _ui_scale,
            _empty_help_width,
            _empty_help_scale_x,
            _empty_help_scale_y,
        ...
        // Строка точных цифр на текущем уровне (пакет №217).
        var _stats_line = tablet_staff_skill_stats_line(_help_id, _target);
        var _desc_y = _help_y1 + 20 * _ui_scale;

        if (_stats_line != "") {
            draw_set_color(make_color_rgb(40, 100, 55));
            draw_text_transformed(
                _help_draw_x,
                _help_y1 + 19 * _ui_scale,
                _stats_line,
                0.56 * _font_ui,
                0.62 * _font_ui,
                0
    ...
    if (
        ...
        var _has_inpatient = instance_exists(obj_inpatient_controller);
        var _has_operating = instance_exists(obj_operating_table);
