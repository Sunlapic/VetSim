...
function wait_bar_fill_rect(_pos, _ratio) {
    ...
    var _fill_w = _full_w * clamp(_ratio, 0, 1);

    // Минимальная «капелька», чтобы обводка не схлопнулась.
    if (_fill_w < 3) _fill_w = 3;
...
/// @function wait_bar_outline_fill(_fill, _thickness)
/// @description Тонкая чёрная обводка ВОКРУГ ЗАЛИВКИ (пакет №331).
///              Раньше обводилась вся дорожка — теперь обнимается
///              сам цвет, и полоска выглядит капсулой.
function wait_bar_outline_fill(_fill, _thickness) {

    var _t = _thickness;
    var _h = (_fill.y2 - _fill.y1) + _t * 2;
    var _w = (_fill.x2 - _fill.x1) + _t * 2;
    var _r = min(_h * 0.5, _w * 0.5);

    draw_set_alpha(1);
    draw_set_color(make_color_rgb(18, 16, 14));
    draw_roundrect_ext(
        _fill.x1 - _t, _fill.y1 - _t, _fill.x2 + _t, _fill.y2 + _t,
        _r, _r,
        true
    );
}
