...
// ПАКЕТ №295: было `is_hovered && mouse_check_button_pressed(mb_left)` —
// то есть карточка открывалась в момент КАСАНИЯ экрана, когда ещё
// неизвестно, тап это или начало перетягивания камеры. На телефоне
// достаточно было начать жест пальцем на кандидате, чтобы поверх
// карты выскочила его карточка.
//
// Это ровно тот баг, который пакет №264 уже исправил для всех
// остальных персонажей (par_staff, par_visitors, par_animals) — но
// кандидата тогда пропустили: у него собственный обработчик клика,
// потому что par_staff открывает планшет и кандидата явно исключает
// (`if (!_is_candidate)`), а кандидату нужна своя компактная карточка.
//
// world_tap_on_me() ждёт ПОДТВЕРЖДЁННОГО тапа: палец поднялся, не
// сдвинувшись больше чем на 12 пикселей, и точка отпускания попала
// по спрайту. Тем же механизмом пользуется obj_player, поэтому герой
// при панорамировании никуда не идёт.
if (
