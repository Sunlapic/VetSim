...
switch (state) {
    ...
    case "going_to_exam":
        ...
        if (exam_walk_timer >= game_get_speed(gamespeed_fps) * 6) {
            exam_walk_timer = 0;
            exam_walk_retries += 1;
    ...
    case "in_exam":
        ...
        if (_orphaned) {
            ...
            if (exam_orphan_timer >= game_get_speed(gamespeed_fps) * 5) {
                exam_orphan_timer = 0;
...
var _patience_max_frames = round(_wait_seconds * game_get_speed(gamespeed_fps));
...
if (_is_waiting) {
    ...
    if (!patience_xp_awarded_this_visit) {
        ...
        if (patience_wait_xp_timer >= game_get_speed(gamespeed_fps) * 10) {
            ...
            if (_patience_level_up) {
                ...
                var _new_patience_max = round(_new_wait_seconds * game_get_speed(gamespeed_fps));
...
if (state == "leaving_clinic") {
    ...
    if (leave_stuck_timer >= game_get_speed(gamespeed_fps) * 4) {
        leave_stuck_timer = 0;
