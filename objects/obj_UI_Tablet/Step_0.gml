...
// Пакет №288: таймеры переворота листа и вспышки диагноза.
// Считаются здесь, а не в Draw: шаг вызывается ровно раз за кадр,
// а отрисовка может пропускаться или повторяться.
if (card_flip_timer > 0) {
    card_flip_timer -= 1;
}

if (card_diagnosis_flash_timer > 0) {
    card_diagnosis_flash_timer -= 1;
}
