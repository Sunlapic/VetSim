...
// ═══════════════════════════════════════════════════════════════
// ПАКЕТ №296: ЧТО СЧИТАТЬ ОТКАЗОМ ОТ ПРИЁМА
//
// Раньше здесь стоял mouse_check_button_pressed(mb_left) — момент
// КАСАНИЯ экрана. На телефоне это отменяло приём при любом жесте:
// потянул карту, свёл пальцы для зума, ткнул в персонажа посмотреть
// карточку — врач бросал вызов и вставал.
//
// Причина та же, что чинил пакет №264 для карточек: касание — ещё не
// команда. Пока палец не поднялся, неизвестно, тап это или начало
// перетягивания.
//
// Теперь отмена требует ВСЕХ трёх условий:
//
//   1. Подтверждённый тап (world_tap_happened): палец поднялся, не
//      сдвинувшись. Перетягивание и щипок отсеиваются механизмом
//      пакета №141 — тем же, из-за которого герой не идёт за камерой.
//
//   2. Тап пришёлся НЕ по персонажу. Тык в человека — это «покажи
//      карточку», а не «отмени приём». Проверка ровно та же, что уже
//      стоит ниже в блоке свободного движения (_clicked_interactive_actor).
//
//   3. Мир не перекрыт интерфейсом — как и было.
//
// Итог: приём отменяется только когда игрок действительно указал
// врачу идти в другое место — то есть тапнул по свободному полу.
// ═══════════════════════════════════════════════════════════════

var _cancel_tap_on_actor = (
        ...
        if (point_distance(x, y, assigned_owner.x, assigned_owner.y) <= 40) {
            ...
            if (instance_exists(assigned_pet)) {
                with (assigned_pet) {
                    ...
                    // животное отправляют В КАБИНЕТ: здесь, у врача и у
                    // ассистента. Именно этим путём собака и шла сквозь
                    // стену — с приёма её ведёт этот код, а не стационар.
                    //
                    // animal_walk_to при недостижимой цели ищет соседнюю
                    // проходимую клетку, а не идёт напролом.
