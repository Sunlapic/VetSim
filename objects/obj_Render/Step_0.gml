...
// ═══════════════════════════════════════════════════════════════
// 3.2 DEBUG: КЛАВИША R — ТЕСТОВАЯ КОМНАТА (пакет №302)
//
// room1 больше не участвует в игровом цикле: игра начинается в
// маленькой клинике rm_clinic. Но room1 осталась как полигон, где
// собраны все системы разом — стационар, операционная, три кабинета.
// Проверять на ней новые механики удобнее, чем разворачивать всё
// заново.
//
// Клавиша R переключает между текущей клиникой и полигоном.
// Свободные буквы проверены: N, O, P, Q, W уже заняты отладкой.
//
// Персонал и склад перед уходом сворачиваются в карман клиники
// (пакет 301) — иначе сотрудники, нанятые в клинике, потерялись бы
// при первом же заходе на полигон.
//
// Как и вся отладка, работает только при global.vetsim_debug_mode.
// ═══════════════════════════════════════════════════════════════
...
if (
    ...
    // ПАКЕТ №306: имя ресурса пишется РОВНО как в дереве проекта —
    // "Room1" с большой буквы. asset_get_index регистрозависим, и с
    // "room1" он возвращал -1: клавиша молча ничего не делала, а
    // клиника №4 сообщала «помещение не готово».
    var _test_room = asset_get_index("Room1");

    if (_test_room != -1 && room_exists(_test_room)) {
        if (room == _test_room) {
            // Возвращаемся из полигона в свою клинику.
            var _home_id = variable_global_exists("active_clinic")
                ? global.active_clinic
                : 1;

            var _home = clinics_get(_home_id);

            if (
                is_struct(_home)
                && string(_home.room_name) != ""
                && asset_get_index(_home.room_name) != -1
            ) {
                global.clinic_restore_pending = true;
                global.clinic_room_transition = true;
                room_goto(asset_get_index(_home.room_name));
            }
        }
        else {
            // Уходим на полигон, свернув свою клинику.
            clinic_network_store_current();

            // Флаг НЕ ставим: в room1 персонал уже расставлен в
            // редакторе, разворачивать поверх него карман не нужно.
            global.clinic_restore_pending = false;
            global.clinic_room_transition = true;

            room_goto(_test_room);
        }
    }
}
...
if (render_last_day != global.game_day) {
    ...
    // ═══════════════════════════════════════════════════════════
    // ПАКЕТ №301: ДОХОД ОСТАЛЬНЫХ КЛИНИК СЕТИ
    //
    // Купленные клиники, где игрока нет, работают «на бумаге»:
    // приёмы там не разыгрываются, но раз в день капает
    // income_per_day из данных клиники.
    //
    // Клиника, в которой игрок сейчас, по этой статье не платит —
    // её выручка идёт с реальных приёмов, иначе доход задвоился бы.
    // Пустая клиника без сотрудников не приносит ничего.
    //
    // Точка выбрана рядом с автосохранением: это уже готовая ветка
